...
// оттестить нахуй
void S21Matrix::CalcSubMatrix(size_t row, size_t col, const S21Matrix& minor) {
	for (size_t min_x = 0, i = 0; i < minor.rows_; ++i) {
		if (i == row) continue;
		for (size_t min_y = 0, j = 0; j < minor.cols_; ++j) {
			if (j == col) continue;
...
double S21Matrix::CalcDet() {
	...
	if (rows_ == 1)
			...
			for (int i = 0; i < cols_; ++i) {
				...
				S21Matrix minor(rows_ - 1, cols_ - 1);
				for (size_t i = 0, sign = 1; i < rows_; ++i, sign *= -1) {
