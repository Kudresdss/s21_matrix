...
bool EqMatrix(S21Matrix &matrix, double **test_matrix, int rows, int cols) {
  for (int i = 0; i < rows; ++i) {
    for (int j = 0; j < cols; ++j) {
      if (test_matrix[i][j] != matrix(i, j)) return (false);
    }
  }
  return (true);
}

void InitS21Matrix(S21Matrix &matrix, double **test_matrix, int rows,
                   int cols) {
  for (int i = 0; i < rows; ++i) {
    for (int j = 0; j < cols; ++j) matrix(i, j) = test_matrix[i][j];
  }
}
...
void DeleteTestMatrix(double *test, double **test_matrix, int rows) {
  for (int i = 0; i < rows; ++i) delete[] test_matrix[i];
  delete[] test_matrix;
  delete[] test;
}
